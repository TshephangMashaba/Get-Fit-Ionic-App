...
@Component({
  selector: 'app-recovery-flow-status',
  templateUrl: './recovery-flow-status.page.html',
  styleUrls: ['./recovery-flow-status.page.scss'],
  standalone: false
})
export class RecoveryFlowStatusPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentUser) {
      ...
      this.isCompleted = list.includes('recovery-flow');
    }
  }
  ...
  async resetProgress() {
    if (this._storage && this.storageKey) {
      ...
      list = list.filter((p: string) => p !== 'recovery-flow');
